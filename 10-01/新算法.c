...
int TwodimensionRW( int a,int m,int r,int q,int N){/*Schrage方法生成以连续两个随机数实现点的二维随机行走*/
    FILE *fp;/*存放随机数*/
...
int ThreedimensionRW( int a,int m,int r,int q,int N){/*Schrage方法生成以连续两个随机数实现点的三维随机行走*/
    FILE *fp;/*存放随机数*/
    double x;/*x是随机数*/ 
    int n;/*循环变量*/ 
    int I;/*随机数生成的In*/
...
int main() {
    ...
    h = OnedimensionRW(a,m,r,q,N);/*生成N个随机数并保存*/
    if( h != 0 )/*自动报错*/
    printf("Failed to render 1D RW.");
    
    h = TwodimensionRW(a,m,r,q,N);/*生成N个随机数并保存*/
    if( h != 0 )/*自动报错*/
    printf("Failed to render 2D RW.");
	
	h = ThreedimensionRW(a,m,r,q,N);/*生成N个随机数并保存*/
    if( h != 0 )/*自动报错*/
